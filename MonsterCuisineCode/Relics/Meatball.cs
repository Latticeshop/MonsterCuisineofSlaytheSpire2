...
/// <summary>肉丸（仅有肉度 2~3.5 合成）：前两个回合开始时多抽1张牌、多获得1点能量。</summary>
public sealed class Meatball : RelicModel
{
    ...
    public override string PackedIconPath => ResourceLoader.Exists(ModIconPath)
        ? ModIconPath
        : ModelDb.Relic<Vajra>().PackedIconPath;
    protected override string PackedIconOutlinePath => PackedIconPath;
    protected override string BigIconPath => PackedIconPath;

    public override decimal ModifyHandDraw(Player player, decimal count)
    {
        if (player != Owner || Owner.PlayerCombatState?.TurnNumber > 2)
        {
            return count;
        }
        return count + 1m;
        ...
        ICombatState combatState)
    {
        if (participants.Contains(Owner.Creature) &&
            Owner.PlayerCombatState?.TurnNumber is 1 or 2)
        {
            Flash();
